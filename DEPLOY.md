...
```bash
git clone https://github.com/longxingze0925/license-server-ai.git /opt/license-server-ai
...
| `INSTANCE_NAME` | 部署实例名，隔离容器/数据卷/网络 | 可选 | `license-server-ai` |
...
```bash
# 添加 cron 任务
echo "0 2 * * * root /opt/license-server-ai/ssl-manager.sh renew" > /etc/cron.d/certbot-renew
```
...
# 检查网络
docker network ls
docker network inspect license-server-ai-network
```
